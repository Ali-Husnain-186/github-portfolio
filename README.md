...
Personal portfolio site. One self-contained `index.html` — no build step, no framework,
...
| Experience | Vertical timeline — CanDev (current), MindsTek, Artus AI, Nexthon ×2 |
...
| Skills | Frontend / Backend / Engineering tag groups + Education + Languages |
| Contact | Email + LinkedIn CTA, location, phone |
...
- `JOBS` — work history. Each entry: `role`, `company`, `date`, `location`, `type`,
  `desc`, `points[]`, and optional `current: true` for the "Current" pill.
- `FEATURED` — the big GameMania card: `name`, `sub`, `desc`, `bullets[]`, `tags[]`,
  `live`, `code`, `art`.
- `PROJECTS` — the grid. Each entry: `name`, `sub`, `cats[]` (drives the filter chips
